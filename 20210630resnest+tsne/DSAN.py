...
def train(epoch, model):
    ...
    #tsne_sourse_all = torch.zeros(1000)
    #tsne_target_all = torch.zeros(1000)
    ...
    for i in range(1, num_iter):
        ...
        label_source_pred, loss_mmd= model(data_source, data_target, label_source)  # 新增第三个返回值，是用来tsne的数据，但是在训练过程中无用，故而没有后续调用
        #label_source_pred, loss_mmd, tsne_sourse, tsne_target,t_label= model(data_source, data_target, label_source)  # 新增第三个返回值，是用来tsne的数据，但是在训练过程中无用，故而没有后续调用
        if i == 1:
            tsne_sourse_all = model.tsne_sourse
            tsne_target_all = model.tsne_target
            lable_source_all = label_source
            lable_target_all = model.target_label
        else:
            tsne_sourse_all  = torch.cat((tsne_sourse_all  ,model.tsne_sourse) ,0)
            tsne_target_all  = torch.cat((tsne_target_all  ,model.tsne_target) ,0)
            lable_source_all = torch.cat((lable_source_all ,label_source)      ,0)
            lable_target_all = torch.cat((lable_target_all ,model.target_label),0)
    ...
    return tsne_sourse_all, tsne_target_all, lable_source_all, lable_target_all
...
def test(model):
    ...
    with torch.no_grad():
        ...
        for data, target in target_test_loader:
            ...
            s_output, t_output= model(data, data, target)  # 新增第三个返回值，是用来tsne的数据，在测试过程中是有用的。
...
if __name__ == '__main__':
    ...
    for epoch in range(1, epochs + 1):
        ...
        tsne_source_all, tsne_target_all, lable_source_all, lable_target_all = train(epoch, model)
        if epoch == 1:
            final_tsne_source = tsne_source_all
            final_tsne_target = tsne_target_all
            final_label_source = lable_source_all
            final_label_target = lable_target_all
        ...
        print('source: {} to target: {} max correct: {} max accuracy{: .6f}%\n'.format(
              source_name, target_test_name, correct, 100. * correct / len_target_dataset ))
        # 结束计时时间-开始计时时间=模型从开始运行到当次迭代总计用时。
        print('cost time:', end_time - time_start)
    #print(final_tsne_source.size())
    #print('---------------------------------')
    print(final_label_source.size())
    print(final_label_target.size())
    tsne = TSNE(n_components = 2,init = 'pca',random_state=0)
    final_tsne_source = tsne.fit_transform(final_tsne_source.cpu().detach().numpy())
    final_label_source = final_label_source.cpu().detach().numpy()
 

    plt.scatter(final_tsne_source[:, 0], final_tsne_source[:, 1], c=final_label_source, cmap=plt.cm.Spectral)
    plt.title("t-SNE")
